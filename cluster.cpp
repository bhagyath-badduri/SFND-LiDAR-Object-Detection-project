...
pcl::visualization::PCLVisualizer::Ptr initScene(Box window, int zoom)
{
    ...
    // draw window bounding box (wireframe so it won't cover points)
    viewer->addCube(window.x_min, window.x_max,
...
int main()
{
    ...
    for (std::vector<int> cluster : clusters)
    {
        ...
        std::string name = "cluster" + std::to_string(clusterId);
        renderPointCloud(viewer, clusterCloud, name, colors[clusterId % 3]);

        // make points bigger (helps you match instructor view)
        viewer->setPointCloudRenderingProperties(
            pcl::visualization::PCL_VISUALIZER_POINT_SIZE, 6, name
        );

        clusterId++;
    }

    // If clustering returns nothing, show raw data
    if (clusters.size() == 0)
    {
        renderPointCloud(viewer, cloud, "data");
        viewer->setPointCloudRenderingProperties(
            pcl::visualization::PCL_VISUALIZER_POINT_SIZE, 6, "data"
        );
    }
